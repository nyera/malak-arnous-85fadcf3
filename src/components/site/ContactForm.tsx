...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <Field label="What are you working toward?" error={errors.message?.message}>
        <textarea {...register("message")} rows={5} className={input} placeholder="Tell us a little about your goals..." />
      </Field>
      ...
      <AnimatePresence>
        {state === "success" && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-center gap-3 p-4 rounded-sm bg-ember/10 border border-ember/30">
            ...
            <p className="text-sm">Got it — we'll reply within 24 hours.</p>
          </motion.div>
        )}
        {state === "error" && (
