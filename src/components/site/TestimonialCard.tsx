...
export function TestimonialCard({ t, index = 0 }: { t: TestimonialT; index?: number }) {
  return (
    ...
    >
      <Quote className="w-6 h-6 text-ember/60 mb-4 rtl:-scale-x-100" />
      ...
      <figcaption className="flex items-center gap-3 pt-5 border-t border-border">
        <img src={t.avatar} alt={t.name} loading="lazy" className="w-11 h-11 rounded-full object-cover ring-2 ring-ember/30 shrink-0" />
